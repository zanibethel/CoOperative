...
export const INTEGRATION_COMPATIBILITY_RULES: readonly CompatibilityRule[] = [
  ...
  {
    id: "hermes-guard-hydrate-uv-runtime-before-source-discovery",
    targets: ["hermes-agent", "vercel-sandbox"],
    component: "Pinned Hermes one-shot source discovery",
    appliesTo: "Prepared runtimes where the Hermes launcher is uv/env managed and its package environment may be materialized lazily",
    symptom: "The pre-model guard cannot find hermes_cli/oneshot.py even though the Hermes executable exists and has worked previously.",
    rootCause: "The launcher can resolve or materialize its managed environment only when invoked, while a static filesystem search runs too early.",
    knownGoodPattern: "Invoke hermes --version as a deterministic no-model hydration step, then search the disposable sandbox filesystem for hermes_cli/oneshot.py while pruning virtual kernel filesystems. Fail closed before model execution if it still cannot be located.",
    avoidPatterns: [
      "assuming the uv-managed package tree already exists before the launcher is invoked",
      "treating --version hydration as a paid model call",
      "searching only a guessed subset of runtime roots",
    ],
    enforcementPaths: [
      "lib/workflow/linked-project-hermes.ts",
      "tests/linked-project-hermes.test.ts",
  ...
  {
    id: "targeted-repair-lineage-must-survive-generic-composer",
    targets: ["linked-project", "supabase"],
    component: "Targeted repair task lineage",
    appliesTo: "Owner-console repairs whose prompt explicitly references a preserved failed task",
    symptom: "The task text says to repair a preserved patch, but repairSourceTaskId is null and CoOperative launches a generic Hermes patch instead of seeding the preserved patch.",
    rootCause: "Repair lineage lived only in hidden UI metadata, so using a generic composer or alternate action path could drop it.",
    knownGoodPattern: "When a linked-project Hermes description explicitly says 'preserved patch from task <UUID>', infer that UUID only as repair lineage, then run the same server-side validation requiring a failed same-playbook task with a preserved patch.",
    avoidPatterns: [
      "trusting prompt text without validating the referenced source task",
      "depending only on button-local metadata for repair lineage",
      "silently converting a targeted repair into a fresh feature generation",
    ],
    enforcementPaths: [
